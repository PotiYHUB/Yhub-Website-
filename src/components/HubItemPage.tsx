        ...
        <article className="bg-white rounded-3xl overflow-hidden border border-slate-100 shadow-lg" id={`hub-article-${item.id}`}>
          ...
          <div className="p-6 sm:p-10 md:p-12">
            ...
            <div className="flex items-center space-x-2 text-xs sm:text-sm text-slate-400 font-semibold mb-4">
              ...
              <span>გამოქვეყნდა: {item.date ? formatDisplayDate(item.date.substring(0, 10)) : ''}</span>
            </div>
                ...
                {item.deadline && (
                  <div className="flex items-start space-x-3">
                    ...
                    <div>
                      ...
                      <span className="font-mono text-sm font-semibold text-slate-800">{formatDisplayDate(item.deadline)}</span>
                    </div>
                  </div>
                )}
                {item.location && (
